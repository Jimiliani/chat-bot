...
class MobileProxy:
    ...
    def as_dict(self):
        print(f'[{self._id}]Пытаемся использовать прокси')
        while self._used_ips.get(self.ip, datetime.datetime.min) + datetime.timedelta(minutes=10) > datetime.datetime.now():
            ...
            print(f'[{self._id}]IP {self.ip} был использован {seconds // 60} минут {seconds % 60} секунд назад')
            print(self._used_ips.get(self.ip, datetime.datetime.min))
            print(datetime.datetime.now())
            self.change_ip()
        self._update_used_ip()
        print(f'[{self._id}]Используем прокси с IP {self.ip}')
        return {
    ...
    def change_equipment(self):
        while True:
            print(f'[{self._id}]Получаем новое оборудование')
    ...
    def change_ip(self):
        retries = 3
        while True:
            print(f'[{self._id}]Получаем новый IP')
            response = self._send_request(settings.ProxyMethods.CHANGE_IP)
            new_ip = response.json().get('new_ip', None)
            if new_ip is None:
                print(f'[{self._id}]Плохой ответ, повторяем запрос')
                continue
            if new_ip != self.ip:
                print(f'[{self._id}]Новый IP - {new_ip}')
                self.ip = new_ip
                break
            elif retries == 0:
                ...
                retries -= 1
            print(f'[{self._id}]Новый IP совпадает со старым, повторяем запрос')

    def get_ip(self):
        while True:
            print(f'[{self._id}]Получаем текущий IP')
            ...
            if response.json().get('proxy_id') is not None:
                ...
                if ip:
                    print(f'[{self._id}]Текущий IP - {ip}')
                    return ip
            print(f'[{self._id}]Полученный IP - невалидный, повторяем запрос')
